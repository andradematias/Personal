...
var ProcessFunction = function () {
    ...
    var requestObject = $.ajax({
        type: "POST",
        url: VV.BaseAppUrl + 'api/v1/' + VV.CustomerAlias + '/' + VV.CustomerDatabaseAlias + '/scripts?name=BillingOptionSave',
        ...
        if (resp.data[0] != 'undefined') {
            ...
            if (resp.data[0] == 'Success') {
                VV.Form.DoAjaxFormSave().then(function () {
                    VV.Form.Global.DisplayMessaging("Billing Option was saved.", "Billing Option");
                    VV.Form.HideLoadingPanel();
                });
            }
            else if (resp.data[0] == 'Error') {
                messageData = 'An error was encountered. ' + resp.data[1];
                VV.Form.Global.DisplayMessaging(messageData, "Billing Option");
            }

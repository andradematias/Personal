    ...
    //If the start date is not empty
        ...
        if (VV.Form.GetFieldValue('End Date')) {
            if (VV.Form.Global.CentralDateValidation(VV.Form.GetFieldValue('Start Date'), 'DateBefore', VV.Form.GetFieldValue('End Date'), 'D', 1) != true) {
                VV.Form.SetValidationErrorMessageOnField('Start Date', 'A Start Date has to be selected being at least one day before End Date')
                ErrorReporting = false;
            }
            else {
...
if (ControlName == 'Option Type' || RunAll) {
    if (VV.Form.Global.CentralValidation(VV.Form.GetFieldValue('Option Type').trim(), 'DDSelect') == false) {
        VV.Form.SetValidationErrorMessageOnField('Option Type', 'Please select a Option Type from the dropdown list.');
        ErrorReporting = false;
    }
    else {
...
if (VV.Form.GetFieldValue('Subscription Pack ID')) {
    ...
    if (VV.Form.Global.CentralNumericValidation(VV.Form.GetFieldValue('Amount'), 0, 'GreaterThan') == false) {
        VV.Form.SetValidationErrorMessageOnField('Amount', 'A number needs to be entered for the Amount.');
        ErrorReporting = false;
    }
    else {
    ...
    if (VV.Form.Global.CentralValidation(VV.Form.GetFieldValue('Unit Type').trim(), 'DDSelect') == false) {

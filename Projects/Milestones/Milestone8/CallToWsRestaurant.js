...
var ProcessFunction = function () {
    ...
    var requestObject = $.ajax({
        type: "POST",
        url: VV.BaseAppUrl + 'api/v1/' + VV.CustomerAlias + '/' + VV.CustomerDatabaseAlias + '/scripts?name=WsRestaurant',
    ...
    if (typeof (resp.status) != 'undefined') {
        ...
        if (resp.data[0] != 'undefined') {
            ...
            if (resp.data[0] == 'Success') {
                ...
                VV.Form.SetFieldValue('Bill ID', resp.data[2], true);

                VV.Form.DoAjaxFormSave().then(function (resp) {
                    VV.Form.HideLoadingPanel();
                    VV.Form.Global.DisplayMessaging('The Order has been saved successfully.', "Success");
                })
